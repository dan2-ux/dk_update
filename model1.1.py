...
@tool
def time_teller(time : str):
    """ Time teller function """
    time = datetime.now().strftime("%H:%M:%S %p")
    return time

@tool
def date_teller(date : str):
    """ Date teller function """
...
def model_call(state: AgentState) -> AgentState:
    system_prompt = SystemMessage(content="""
        ...
        There are also couple of things like time and date, which have tools for you to call if user asking for those.
    """)

    # Get last user input content
    last_user_input = ""
    ...
    if is_tool_needed:
        ...
        if response:
            ...
            print("\nAI1: [No meaningful response generated]")
...
while True:
    user_input = input("\nEnter: ")
    if user_input in ["end", "exit", "clode"]:
        print("Closing")
        break
